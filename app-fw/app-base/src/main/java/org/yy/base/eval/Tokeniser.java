...
//            if(this.position < len - 1 &&  this.string.charAt(this.position + 1) == '('){
//               return Operator.createFunction(name);
//            }
    ...
    private StringValue getString() {
        final int len = this.string.length();
        final int start = this.position;
        char ch = '\0';

        while (this.position < len && (ch = this.string.charAt(this.position)) != '\'') {
            this.position++;
        }
        if (ch != '\'') {
            throw new RuntimeException("' is missing in " + this.string);
        }
        String str = this.string.substring(start, this.position);
        this.position++;
